 ...
 */
import { resolveQQBotAccess } from "./access.js";
...
// ============================================================================
export function runAccessStage(event, accountConfig) {
...
// ============================================================================
export function buildUserContent(event, processed) {
...
function resolveGroupConfig(accountConfig, groupOpenid) {
...
function detectWasMentioned(event) {
    // Check mentions[].is_you
    if (event.mentions?.some((m) => m.is_you))
        return true;
    // Check eventType
    if (event.eventType === "GROUP_AT_MESSAGE_CREATE")
        return true;
    return false;
}
...
function resolveImplicitMention(event) {
    // 引用了机器人的消息 → 视为已 @
...
// ============================================================================
export function buildQuotePart(replyTo) {
    if (!replyTo)
        return "";
    return replyTo.body ? `[Quoted message begins]\n${replyTo.body}\n[Quoted message ends]\n` : `[Quoted message begins]\nOriginal content unavailable\n[Quoted message ends]\n`;
}
export function buildUserMessage(event, userContent, quotePart, groupInfo) {
    const isGroupChat = event.type === "group" || event.type === "guild";
    if (isGroupChat) {
...
export function buildAgentBody(event, userMessage, dynamicCtx) {
    // 斜杠命令跳过装饰
...
// ============================================================================
export function buildDynamicCtx(processed) {
    const lines = [];
...
// ============================================================================
export async function buildInboundContext(event, accountConfig) {
    // Stage 1: Access
