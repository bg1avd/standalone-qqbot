 ...
 */
export async function refreshAccessToken(appId, clientSecret) {
 ...
 * 失败后抛出错误，由调用方决定是否重试。
 */
async function fetchNewToken(appId, clientSecret) {
    ...
    const resp = await httpRetry(() => fetch(`${TOKEN_URL}${TOKEN_PATH}`, {
        method: "POST",
 ...
 */
export function startBackgroundTokenRefresh(appId, clientSecret, callbacks) {
 ...
 * 清理所有 token（退出时调用）。
 */
export function flushAllTokens() {
