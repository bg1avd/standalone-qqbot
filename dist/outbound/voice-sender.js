 ...
 * 编码为 SILK
 */
export async function encodeToSilk(wavBuffer) {
 ...
 */
export async function sendVoice(ctx, audioPath, options = {}) {
    ...
    if (size > MAX_ONESHOT_SIZE) {
        ...
        try {
            ...
            }, "voice", audioPath, // 直接传原文件路径让 uploader 读取（避免内存 double）
            fileName, {});
 ...
 */
async function sendVoiceFromDataUrl(ctx, dataUrl, fileName) {
    try {
        // 解析 Base64
 ...
 * 发送语音引用（类似媒体引用）
 */
async function sendVoiceReference(ctx, kind, fileUuid, fileName) {
    if (!fileUuid) {
...
function getTargetResource(targetType) {
    switch (targetType) {
        case "c2c":
        case "dm":
            return "users";
...
//# sourceMappingURL=voice-sender.js.map