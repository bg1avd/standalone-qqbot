...
async function sendMedia(
  ...
  if (isLocal) {
    ...
    if (!resolvedPath.ok) {
      return { channel: "qqbot", error: (resolvedPath as any).error }
    }
 ...
 * 发送本地文件
 */
async function sendMediaFromLocal(
  ctx: MediaContext,
  kind: MediaKind,
  localPath: string,
  fileName: string
 ...
 * 发送 Base64 数据
 */
async function sendMediaFromBase64(
  ctx: MediaContext,
  kind: MediaKind,
  dataUrl: string,
  fileName?: string
 ...
 */
async function sendMediaReference(
  ctx: MediaContext,
  kind: MediaKind,
...
function buildMediaUploadPath(targetType: SendTargetType, kind: MediaKind): string {
...
function buildMessageSendPath(targetType: SendTargetType): string {
  switch (targetType) {
    case "c2c":
...
function getMessageType(kind: MediaKind): number {
  switch (kind) {
    case "image":
...
function getMediaType(kind: MediaKind): number {
  // QQ API media types
  switch (kind) {
    case "image":
      return 1
...
function getMimeType(kind: MediaKind, fileName?: string): string {
  if (fileName) {
...
function getFileSize(path: string): number {
  ...
  return statSync(path).size as number
}
