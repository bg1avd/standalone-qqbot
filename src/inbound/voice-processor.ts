 ...
 * 解码 SILK 音频为 WAV
 */
export async function decodeSILK(silkBuffer: Uint8Array, options: VoiceProcessorOptions = {}): Promise<Uint8Array> {
 ...
 */
export async function transcribeAudio(wavBuffer: Uint8Array, options: VoiceProcessorOptions = {}): Promise<string> {
 ...
 * 便捷函数：从 URL 下载并处理语音
 */
export async function downloadAndProcessVoice(
  url: string,
  options: VoiceProcessorOptions = {}
): Promise<ProcessedVoice> {
  try {
