...
/** 全局引用缓存 */
const refIndexStore = new Map<string, RefEntry & { expiresAt: number }>()
 ...
 * 写入引用缓存
 */
export function setRefIndex(msgIdx: string, entry: Omit<RefEntry, "timestamp"> & { timestamp?: number }): void {
 ...
 * 从缓存读取引用条目
 */
export function getRefIndex(msgIdx: string): RefEntry | undefined {
 ...
 */
export function buildRefEntryFromMessage(msg: QueuedMessage): Omit<RefEntry, "timestamp"> & { timestamp?: number } {
  return {
    content: msg.content,
